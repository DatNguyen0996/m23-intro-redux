...
const RootComponent = (props) => {
  ...
  const addProductToCart = (newProduct) => {
    ...
    setCart({ products: newCartProductList, totalPrice: newTotalPrice });
  };
  const removeProductToCart = (selectProduct) => {
    let newTotalPrice = cart.totalPrice;
    const newCartProductList = cart.products.map((cartProduct) => {
      if (cartProduct.id === selectProduct.id && cartProduct.qty > 0) {
        cartProduct.qty -= 1;
        cartProduct.price -= selectProduct.price;
        newTotalPrice -= selectProduct.price;
      }
      return cartProduct;
    });
    ...
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item md={6}>
          <ProductPage
            products={products}
            addNewProduct={addProductToCart}
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button
              variant="success"
              sx={{ width: "5rem" }}
              onClick={() => props.addNewProduct(props.product)}
            >
              Add
            </Button>
            <Button
